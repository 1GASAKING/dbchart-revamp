...
interface DbmlRef {
  name?: string;
  endpoints: Array<{ entity: string; field: string }>;
}
...
export function parseDbml(input: string): ParseResult {
  ...
  while ((tableMatch = tableRegex.exec(clean)) !== null) {
    ...
    const fields: DbmlColumn[] = [];
    const columnRegex =
      /["`']?([A-Za-z_][\w$]*)["`']?\s+([A-Za-z_][\w$]*(?:\s*\|\s*[A-Za-z_][\w$]*)*)\s*(\[[^\]]*\])?/g;
  ...
  const refRegex =
    /Ref\s+(?:(?:["`']?[\w$]+["`']?)\s*)?\{\s*["`']?([\w$]+)["`']?\s*\.\s*["`']?([\w$]+)["`']?\s*([<>\-])\s*["`']?([\w$]+)["`']?\s*\.\s*["`']?([\w$]+)["`']?\s*\}/g;
  ...
  for (const ref of refs) {
    ...
    const sourceField = sourceEntity.fields.find(
      (f) => f.name.toLowerCase() === a.field.toLowerCase()
    );
