import type { ConnectionConfig } from "./types/connection-config";

export interface DetailedConnectionError {
  message: string;
...
export function normalizeConnectionError(
  ...
  if (anyErr?.sqlState){ details.sqlState = String(anyErr.sqlState);}
  if (anyErr?.errno !== undefined && anyErr?.errno !== code) {details.errno = anyErr.errno;}
  if (anyErr?.syscall) {details.syscall = String(anyErr.syscall);}
  if (anyErr?.address) {details.address = String(anyErr.address);}
  if (anyErr?.port !== undefined && anyErr?.port !== null){ details.port = anyErr.port;}
  ...
  if (responseStatusText) {details.httpStatusText = String(responseStatusText);}
  ...
  if (anyErr?.host) {details.host = String(anyErr.host);}

  // Always reflect the connection target the user requested.
  if (config?.host) {details.host = config.host;}
  if (config?.port !== undefined && config?.port !== null){ details.port = config.port;}
  if (config?.database) {details.database = config.database;}

  const base =
    err instanceof Error ? err.message : err !== undefined && err !== null ? String(err) : "Unknown error";

  const parts: string[] = [base];
  if (details.code) {parts.push(`[code: ${details.code}]`);}
  if (details.syscall) {parts.push(`(${details.syscall})`);}
